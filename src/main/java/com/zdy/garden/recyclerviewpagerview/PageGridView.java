...
public class PageGridView extends RecyclerView {
    ...
    public void setmColums(int colums) {
        this.mColums = colums;
    }
    ...
    int lastScrollX;
    int target;
    int slopPercent;//滑动距离与宽度百分比（滑动多少跳转下一页）

    public void setSlopWidth(int slopPercent) {
        this.slopPercent = slopPercent;
    }
...
//            SCROLL_STATE_SETTLING--2
            if (newState == SCROLL_STATE_DRAGGING) {
                currentPage = scrollX / width;
                lastScrollX = scrollX;
            } else if (newState == SCROLL_STATE_SETTLING) {
                //do nothing
            } else if (newState == SCROLL_STATE_IDLE) {
                if (lastScrollX - scrollX < 0 && Math.abs(lastScrollX - scrollX) > width / slopPercent) {
                    targetPage = currentPage + 1;
                } else if (lastScrollX - scrollX > 0 && Math.abs(lastScrollX - scrollX) > width / slopPercent) {
                    targetPage = currentPage - 1;
                }

                if (pageIndicator != null) {
                    pageIndicator.onPageUnSelected(currentPage);
                    pageIndicator.onPageSelected(targetPage);
                }
                if (onPageChangeListenerList != null) {
                    for (OnPageChangeListener listener : onPageChangeListenerList) {
                        listener.onPageChanged(targetPage);
                    }
                }

                target = targetPage * getWidth();
